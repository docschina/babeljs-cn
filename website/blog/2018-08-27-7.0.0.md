...
最初的问题是，与服务器语言不同，JavaScript 没有办法保证对每个用户都有相同的支持，因为用户可能使用不同版本的浏览器（尤其是旧版本的 Internet Explorer）。如果开发人员想要使用新语法（例如 `class A {}`），旧浏览器上的用户只会因为 `SyntaxError` 的错误而出现屏幕空白的情况。
