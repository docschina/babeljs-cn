...
历经 2 年，4k 多个提交，50 多个预发布版本以及大量社区援助，我们很高兴地宣布发布 Babel 7。自 Babel 6 发布以来，已经过了将近[三年的时间](https://babeljs.io/blog/2015/10/29/6.0.0)！发布期间有许多要进行的迁移工作，因此请在发布第一周与我们联系。Babel 7 是更新巨大的版本：我们使它编译更快，并创建了升级工具，支持 JS 配置，支持配置 "overrides"，更多 size/minification 的选项，支持 JSX 片段，支持 TypeScript，支持新提案等等！
...
最初的问题是，JavaScript 与服务器语言不同，它没有办法保证对每个用户都有相同的支持，因为用户可能使用支持程度不同的浏览器（尤其是旧版本的 Internet Explorer）。如果开发人员想要使用新语法（例如 `class A {}`），旧浏览器上的用户只会因为 `SyntaxError` 的错误而出现屏幕空白的情况。
...
开源的可持续性就像提供了一个资金储备池（offering basket）一样。说明为使用开源的成千上万的人和公司提供价值并不难，但我们并没有关注到那些愿意投入开源工作中少数人的价值。可以通过这些[支持方式](https://github.com/nayafia/lemonade-stand)来支持开源，但并非所有方法都适用于每个项目或人员。
...
我们对一些插件的 `loose` 选项进行了一些更改，并为其他插件添加了一些新选项！注意，在使用这些选项时，你应该了解你在做什么，因为这是一个非规范行为。当你关掉编译去使用原生语法时，这可能会成为一个问题。如果使用的话，这些选项最好在一个库中进行使用。
...
> 这种集成是相当超前的，因此可能不完全支持某些语法。我们非常期待您的帮助，为其[提出 issues 并发送 PR（如果可能的话）](https://github.com/babel/babel/labels/area:%20typescript?page=2&q=is%3Aopen+label%3A%22area%3A+typescript%22)!
...
`external-helpers` 和 `rollup-plugin-babel` 也可以这样做。我们正在研究未来如何自动执行此操作。请留意不久后单独关于 Babel's helpers 的文章。
...
但是它包含完整的 polyfill，如果某些功能浏览器已经支持，你可能不需要全部引入。这与 `@babel/preset-env` 试图用语法解决的问题相同，所以我们将它应用于 polyfill 中。选项 `useBuiltins: "entry"` 通过将原始 import 拆分成仅必要的 import 来解决该问题。

但我们可以通过仅 import 代码库中所使用的 polyfill 来实现会更佳。选项 `"useBuiltIns: "usage"` 是我们第一次尝试这样做（[相关文档](https://babeljs.io/docs/en/babel-preset-env#usebuiltins)）。
...
Babel 一直试图平衡转译和功能对大小的影响，并提供给 JavaScript 的作者。 在 Babel 7 中，配置 Babel 以支持[模块/非模块模式](https://github.com/kristoferbaxter/preset-env-modules)变得更加容易。
...
我们还在使用 [Crowdin](https://crowdin.com/project/babeljs) 为 Babel 官网建立了[翻译](https://github.com/babel/website/issues/1619)，而 Babel 7 的翻译，将是我们更好的起点。

#### REPL

![](https://i.imgur.com/j0qOllj.png)

我们将 [REPL](https://babeljs.io/repl) 重写为 [React 组件](https://github.com/babel/website/tree/master/js/repl)，并且已经与 Ives 合作，更好地与 [CodeSandbox](https://codesandbox.io/) 集成。这允许在 REPL 中可以使用 npm 安装任何插件或 preset ，以及从 [CodeSandbox](https://github.com/CompuIves/codesandbox-client) 获得的任何更新。
...
- Babel 本质上与它编译的内容联系在一起：JavaScript。 只要有新的提案/工作，那就有事情可做。这包括在语法变得"稳定"之前花费时间/精力实现和维护该语法。我们关心整个过程：升级路径，新功能的教育，标准/语言设计的教学，易用性以及与其他项目的集成。
...
我们都很辛苦，但以这种方式服务确实是一种荣幸，所以我们希望你赞赏这一次发布！
